...
class LensReconstruction():
    ...
    def load_image(self, fold_path, str_num, img_type="png",
    ...
    def batch_load_image(self, batch_list, fold_path,
        ...
        for i in range(len(batch_list)):
            ...
            img = self.load_image(fold_path, batch_list[i], img_type=img_type, readmethod=readmethod)
    ...
    def lens2source_discriminator(self):
        ...
        para = Input(shape=(self.para_len,))
        input_layer = Concatenate()([img, lbl])

        # grab the feature of images, labels and paras
        para_dense = Dense(128, activation="relu")(para)
        para_drop = Dropout(.2)(para_dense)

        features = model(input_layer)

        features = Concatenate()([features, para_drop])
        features = Dense(64, activation="relu")(features)
        features = Dropout(.2)(features)
